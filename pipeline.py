...
def analyse_split(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "format": rd.detect_format(df),
        "turn_validation": rd.validate_turn_ordering(df, rd.detect_format(df)),
...
def audit(link: str, row_limit: int | None = None) -> AuditReport:
    ...
    splits = load_dataset(info, hf_split_limit=row_limit)
    if row_limit:
        splits = {k: v.head(row_limit) for k, v in splits.items()}
