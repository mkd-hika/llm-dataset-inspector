...
def load_dataset(
    ...
    row_start: int = 0,
    hf_split_limit: int | None = None,
) -> Dict[str, pd.DataFrame]:
    """Stream/load a dataset into one DataFrame per split.

    row_start: number of rows to skip from the beginning (0 = start from first row).
    row_limit: max rows to read after the offset (None = all remaining rows).
    """
    limit = row_limit or hf_split_limit
    start = max(0, row_start or 0)

    if info.source_type == "huggingface":
        return _load_huggingface(info, limit, start)
    ...
    if info.source_type == "local":
        ...
        df = _read_local(info.resolved_url, info.file_format, info.compression, limit, start)
        return {"data": df}
    ...
    df = _stream_response(resp, info.file_format, info.compression, limit, start)
    return {"data": df}


def _read_local(path: str, file_format: str, compression: str,
                row_limit: int | None, row_start: int = 0) -> pd.DataFrame:
    comp = "gzip" if compression == "gzip" else None
    if file_format == "csv":
        skip = list(range(1, row_start + 1)) if row_start else None
        return pd.read_csv(path, compression=comp, skiprows=skip, nrows=row_limit)
    if file_format == "tsv":
        skip = list(range(1, row_start + 1)) if row_start else None
        return pd.read_csv(path, sep="\t", compression=comp, skiprows=skip, nrows=row_limit)
    if file_format == "parquet":
    ...
    if file_format == "json":
        ...
        return _parse_json_bytes(raw, row_limit, row_start)
    raise ValueError(f"Unsupported file format: {file_format!r}")


def _stream_response(resp, file_format: str, compression: str,
                     row_limit: int | None, row_start: int = 0) -> pd.DataFrame:
    ...
    if file_format == "csv":
        skip = list(range(1, row_start + 1)) if row_start else None
        return pd.read_csv(bio, skiprows=skip, nrows=row_limit)
    if file_format == "tsv":
        skip = list(range(1, row_start + 1)) if row_start else None
        return pd.read_csv(bio, sep="\t", skiprows=skip, nrows=row_limit)
    if file_format == "parquet":
    ...
    if file_format == "json":
        return _parse_json_bytes(content, row_limit, row_start)
    raise ValueError(f"Unsupported file format: {file_format!r}")


def _parse_json_bytes(raw: bytes, row_limit: int | None, row_start: int = 0) -> pd.DataFrame:
...
def _load_huggingface(info: SourceInfo, split_limit: int | None, row_start: int = 0) -> Dict[str, pd.DataFrame]:
    try:
    ...
    for split_name, split_ds in ds.items():
        iterable = split_ds.skip(row_start) if row_start else split_ds
        iterable = iterable.take(split_limit) if split_limit else iterable
